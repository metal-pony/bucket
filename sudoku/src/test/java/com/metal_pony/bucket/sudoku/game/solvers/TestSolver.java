package com.metal_pony.bucket.sudoku.game.solvers;
...
import com.metal_pony.bucket.sudoku.game.solvers.Solver.BoardSolution;
...
public class TestSolver {
  private static final List<String> VALID_CONFIGS = Collections.unmodifiableList(
	...
	private static final List<String> INVALID_CONFIGS = Collections.unmodifiableList(
		Arrays.asList(
			"1".repeat(Board.CELLS),
			"2".repeat(Board.CELLS),
			"3".repeat(Board.CELLS),
			"4".repeat(Board.CELLS),
			"5".repeat(Board.CELLS),
			"6".repeat(Board.CELLS),
			"7".repeat(Board.CELLS),
			"8".repeat(Board.CELLS),
			"9".repeat(Board.CELLS),
  ...
  static {
		INVALID_CELL_INDICES = Collections.unmodifiableList(
			Arrays.asList(
				Integer.MIN_VALUE, -100, -10, -1, Board.CELLS, Board.CELLS + 1,
				Board.CELLS + 10, Board.CELLS + 100, Integer.MAX_VALUE
			)
		);
    INVALID_BOARD_SIZES = IntStream.range(0, Board.CELLS * 2)
			.filter((i) -> i != Board.CELLS)
			.boxed()
  ...
  // TODO #23 test all methods with int[] param when int[].length != CELLS
  ...
  public static int d(int encoded) {
    ...
    int result = 0;
    for (int digit = 1; digit <= Board.DIGITS; digit++) {
  ...
  public static String boardString(int[] board) {
    ...
    for (int rowIndex = 0; rowIndex < Board.DIGITS; rowIndex++) {
      Solver.forEachCellInRow(board, rowIndex, (index, value) -> {
        strb.append(value);

        // Add comma if not last cell in row
        if (index % Board.DIGITS != Board.DIGITS - 1) {
          strb.append(',');
        }
      });

      // Add newline if not last row
      if (rowIndex < Board.DIGITS - 1) {
