...
import com.metal_pony.bucket.sudoku.game.*;

public class Solver {
	static class Constraints {
		static final int ROW_MASK = ALL << (DIGITS * 2);
		static final int COLUMN_MASK = ALL << DIGITS;
		...
		Constraints() {
			this.values = new int[DIGITS];
		}

		Constraints(Constraints source) {
			this();
			System.arraycopy(source.values, 0, this.values, 0, DIGITS);
		}
		...
		int getForRow(int rowIndex) {
			return (values[rowIndex] & ROW_MASK) >> (DIGITS * 2);
		}

		int getForCol(int columnIndex) {
			return (values[columnIndex] & COLUMN_MASK) >> DIGITS;
		}
		...
		void add(int cellIndex, int digit) {
			...
			values[rowIndex] |= 1 << (digit - 1 + (DIGITS * 2));
			values[columnIndex] |= 1 << (digit - 1 + DIGITS);
		...
		String bin(int constraints) {
			...
			if (str.length() < Board.DIGITS) {
				pad = "0".repeat(Board.DIGITS - str.length());
			}
	...
	static int searchForUniqueCandidate(int[] board, int cellIndex) {
		...
		for (int candidateMask = 1; candidateMask < (1<<Board.DIGITS); candidateMask <<= 1) {
	...
	static int[] copyBoard(int[] board) {
		...
		System.arraycopy(board, 0, copy, 0, CELLS);
		return copy;
	}
	...
	public static Board solve(Board board) {
		...
		searchForSolutions(
			new BoardSolution(board),
			(_solution) -> {
				System.arraycopy(_solution, 0, solution, 0, CELLS);
				return false;
			}
		);
	...
	private static int pickEmptyCell(int[] board) {
		int minIndex = -1;
		int minCandidates = Board.DIGITS + 1;
