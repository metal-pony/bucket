...
import com.metal_pony.bucket.sudoku.game.solvers.*;
...
public class Generator {
  ...
  public static List<Board> generatePuzzles2(int numClues, double prob) {
    ...
    int[] _board = new int[Board.CELLS];
  ...
  private static void fillSections(Board board, int mask) {
    int[] list = getSeries(Board.DIGITS, 1);
    for (int m = 0; m < Board.DIGITS; m++) {
      if ((mask & (1 << (8 - m))) > 0) {
        Shuffler.shuffle(list);
        int gr = m/3;
        int gc = m%3;
        for (int i = 0; i < Board.DIGITS; i++) {
